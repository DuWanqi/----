...
export interface EntityBehavior {
  update(deltaTime: number, playerPos: Position, playerNoiseLevel: number, hasLight: boolean, hasTorch?: boolean): void
...
export abstract class BaseEntity implements EntityBehavior {
  ...
  abstract update(deltaTime: number, playerPos: Position, playerNoiseLevel: number, hasLight: boolean, hasTorch?: boolean): void
...
export class SkinStealer extends BaseEntity {
  ...
  private chaseTimer = 0  // 追击计时器
  private readonly maxChaseTime = 10  // 最大追击时间（秒）
  private isFleeing = false  // 是否正在逃离
  private fleeTimer = 0  // 逃离动画计时
  ...
  update(deltaTime: number, playerPos: Position, playerNoiseLevel: number, hasLight: boolean): void {
    ...
    if (this.isFleeing) {
      ...
      const fleeSpeed = 5 * deltaTime
      
      this.position.x += (dx / len) * fleeSpeed
      this.position.z += (dz / len) * fleeSpeed
      this.mesh.position.x = this.position.x
      this.mesh.position.z = this.position.z
      
      // 逐渐缩小消失
      const scale = Math.max(0, 1 - this.fleeTimer / 2)
  ...
  // 获取剩余追击时间
  getRemainingChaseTime(): number {
    if (!this.isRevealed) return this.maxChaseTime
    return Math.max(0, this.maxChaseTime - this.chaseTimer)
  }
  
  // 是否正在追击
  isChasing(): boolean {
    return this.isRevealed && !this.isFleeing
  }
...
export class Shadow extends BaseEntity {
  ...
  update(deltaTime: number, playerPos: Position, playerNoiseLevel: number, hasLight: boolean, hasTorch?: boolean): void {
    ...
    if (hasTorch) {
      // 火把：快速驱散影怪，5倍速度
      this.fadeAmount = Math.max(0, this.fadeAmount - deltaTime * 10)
    } else if (hasLight) {
      // 普通灯光：正常驱散
