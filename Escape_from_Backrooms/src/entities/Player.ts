...
export class Player {
  ...
  private updateLampLight(): void {
    ...
    if (this.state.torchLit && this.torchLight) {
      // 火把闪烁效果（更强烈）
      this.torchLight.intensity = 2.5 + Math.sin(Date.now() * 0.015) * 0.5 + Math.random() * 0.2
    }
  }
  ...
  // 切换火把
  toggleTorch(): void {
    if (!this.state.hasTorch) return
    
    this.state.torchLit = !this.state.torchLit
    
    if (this.state.torchLit) {
      if (!this.torchLight) {
        // 火把光源：更亮、更远、橙红色
        this.torchLight = new THREE.PointLight(0xff6600, 2.5, 12)
        this.torchLight.position.set(0, 1.2, 0)
        this.torchLight.castShadow = false
        this.mesh.add(this.torchLight)
      }
  ...
  hasActiveLight(): boolean {
    return this.state.lampLit || this.state.torchLit
  }

  // 检查火把是否点燃（用于影怪特殊驱散效果）
  isTorchActive(): boolean {
    return this.state.torchLit
  }
